...
LayerControl.prototype.setStatus = function(status) {
  this.status_ =  status;
	...
	function LayerControl(map, div,layer, status) {
  ...
  google.maps.event.addDomListener(AEUI, 'click', function() {
   ...
   var currentStatus = control.getStatus();
	...
	control.setStatus(true);	
				   ...
				   if (currentStatus) {
					   aeText.innerHTML = layer.replace(/_/g, " "); 
						   AEUI.style.backgroundColor = '#1e8fa7';
...
?>
 var layerControlDiv<?php print($cat_clean); ?> = document.createElement('div<?php print($cat_clean); ?>');
  ...
  var layerControl<?php print($cat_clean); ?> = new LayerControl(map, layerControlDiv<?php print($cat_clean); ?>, "<?php print($cat_clean); ?>",true);
...
user-select: none;
  ...
  <body style="margin:0px; padding:0px;" onLoad="load();"> <div style="display:block;overflow:hidden;width:100%">
  ...
  </div>	<div style="font-family:arial;font-size:12px;float:right"> <strong><font color="#003300"><a href="http://trailmaker.org" target="_blank" style="text-decoration:none;color:#003300">powered by trailmaker.org&nbsp;</a></font></strong></div>

    <div><select id="locationSelect" style="width:100%;visibility:hidden"></select></div> </div> <div id="map" style="width: 100%; height: 95%;border:thin #666666 solid"></div> 
